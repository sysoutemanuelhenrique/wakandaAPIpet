...
public interface PetAPI {
	...
	@ResponseStatus (code = HttpStatus.OK)
	PetClienteDetalheReponse getPetClienteComId (@PathVariable UUID clienteID,  @PathVariable UUID petId);
	
	@DeleteMapping  (value = "/{idPet}")
	@ResponseStatus (code = HttpStatus.OK)
	void deletePetById (@PathVariable UUID clienteID,  @PathVariable UUID petId);
	
	@PatchMapping   (value = "/{idPet}")
	@ResponseStatus (code = HttpStatus.OK)
	void patchPet (@PathVariable UUID clienteID, @PathVariable UUID petId,
			@Valid @RequestBody PetEditaRequest petEditaRequest);
